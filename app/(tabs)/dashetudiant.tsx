...
export default function DashboardEtudiant() {
  ...
  const [infoBaccBase, setInfoBaccBase] = useState<{
    numBacc: string;
    serie: string;
    nom: string;
  } | null>(null);
  ...
  useEffect(() => {
    const initDashboard = async () => {
      ...
      if (currentUser) {
        const bacc = listeBacheliersRef.find(
          (b) => b.numBacc === currentUser.numBacc,
        );
        if (bacc) setInfoBaccBase(bacc);
        const sesInscriptions = listeCandidats.filter(
          (c) => c.numBacc === currentUser.numBacc,
        );
  ...
  const getStatusStyle = (statut: string) => {
    switch (statut) {
      case "Inscrit Définitif":
        return { bg: "#27ae60", icon: "checkmark-circle" };
      case "Validé":
        return { bg: "#2980b9", icon: "time" };
      case "Refusé":
        return { bg: "#e74c3c", icon: "close-circle" };
      default:
        return { bg: "#f39c12", icon: "hourglass" };
    }
  };
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <View style={{ marginLeft: 10 }}>
              <Text style={styles.welcomeText}>
                Bonjour, {infoBaccBase?.nom || user}
              </Text>
              <Text style={styles.idText}>
                BACC: {infoBaccBase?.numBacc || "N/A"} • Série{" "}
                {infoBaccBase?.serie || "N/A"}
              </Text>
            </View>
          </TouchableOpacity>
      ...
      <View style={styles.statusSection}>
        <Text style={styles.sectionTitle}>
          Mes Candidatures ({mesInscriptions.length})
        </Text>
      ...
      <View style={styles.gridContainer}>
        <View style={styles.row}>
          <TouchableOpacity
            ...
            onPress={() =>
              router.push({ pathname: "/concours", params: { user } })
            }
          >
          ...
          <TouchableOpacity
            style={[
              styles.menuItem,
              mesInscriptions.length === 0 && { opacity: 0.5 },
            ]}
            onPress={() =>
              mesInscriptions.length > 0 &&
              router.push({ pathname: "/paiement", params: { user } })
            }
          >
        ...
        <View style={styles.row}>
          <TouchableOpacity
            style={styles.menuItem}
            onPress={() =>
              router.push({ pathname: "/notifications", params: { user } })
            }
          >
          ...
          <TouchableOpacity
            style={styles.menuItem}
            onPress={() =>
              router.push({ pathname: "/profil", params: { user } })
            }
          >
